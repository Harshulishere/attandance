...
export async function loadOwnerData(ownerId) {
  ...
  const students = studentsRes.data.map((s) => ({
    ...
    position: s.position || "",
    dob: s.dob || "",
    feeStatus: s.fee_status || "Unpaid",
    feeAmount: s.fee_amount ?? "",
    feeNotes: s.fee_notes || "",
  }));
...
export async function createStudent(ownerId, data) {
  const { error } = await supabase.from("students").insert({
    ...
    fee_amount: data.feeAmount === "" ? null : data.feeAmount,
    fee_notes: "",
  });
  if (error) throw error;
}
export async function updateStudent(studentId, patch) {
  ...
  if ("feeStatus" in patch) dbPatch.fee_status = patch.feeStatus;
  if ("feeAmount" in patch) dbPatch.fee_amount = patch.feeAmount === "" ? null : patch.feeAmount;
  if ("feeNotes" in patch) dbPatch.fee_notes = patch.feeNotes;
